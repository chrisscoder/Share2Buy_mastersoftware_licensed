<div class="form-group" data-ng-class="{'has-error':!modsvar.{{$name}}.$valid && modsvar.{{$name}}.$dirty}">
  ...
  <div class="inner-form-group">
    <input type="password" name="{{$name}}" class="form-control" data-ng-model="{{$name}}" data-ng-init="{{$name}} = <?= htmlspecialchars(json_encode(isset($old) ? $old : null)); ?>" data-ng-minlength="8" data-ng-maxlength="64" data-ng-pattern="/(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z])/" {{ isset($autofocus) == true ? 'autofocus' : '' }} {{ isset($required) ? '' : 'required' }}>
